...
ShaderRGBA::ShaderRGBA(GlobalContexts *global_context) {
    gl_program = -1;
    gl_uMatrix = -1;
    gl_position = -1;
    gl_textCoord = -1;
...
GLuint ShaderRGBA::LoadShader(GLenum type, const char *shaderSrc) {
...
GLuint ShaderRGBA::LoadProgram(const char *vShaderStr, const char *fShaderStr) {
    LOGD("LoadProgram in");
    GLuint vertexShader;
...
int ShaderRGBA::createProgram() {
    ...
    LOGD("CreateProgram : gl_program: %d  image_width: %d, image_height： %d, window_width：%d, window_height：%d",
         gl_program, gl_image_width, gl_image_height, gl_window_width, gl_window_height);
    return gl_program;
}

void ShaderRGBA::render(unsigned char *data) {
    this->rgba_data = data;
...
void ShaderRGBA::setImageSize(int width, int height) {
    this->gl_image_width = width;
    this->gl_image_height = height;
}

void ShaderRGBA::setWindowSize(int width, int height) {
    this->gl_window_width = width;
    this->gl_window_height = height;
}

void ShaderRGBA::initDefMatrix() {
...
void ShaderRGBA::orthoM(float *m, int mOffset, float left, float right, float bottom, float top,
